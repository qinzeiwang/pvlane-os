import {IconArrowForwardUp,IconForbid,IconCheck,IconBuildingWarehouse,IconHome,IconMap,IconDots,IconAdjustmentsHorizontal,IconPointer,IconArrowLeft,IconTrash,IconPlus,IconFolderOpen,IconDeviceFloppy,IconFileText,IconBox,IconSolarPanel,IconLayoutGrid,IconAngle,IconArrowsHorizontal,IconLayoutGridAdd,IconSun,IconStack2,IconX,IconChevronDown,IconArrowBackUp,IconMaximize,IconInfoCircle,IconMenu2,IconHandMove,IconZoomIn,IconZoomOut} from '@tabler/icons-react';
...
// Domain-specific symbols follow Tabler's 24px / 1.75px outline contract.
const roofPaths:Record<string,string>={
 'ridge-direction':'M3 12h18 M3 8v8 M21 8v8',
 'slope-direction':'M3 14.5h18v-5Z',
...
export function Icon({name,style}:{name:string;style?:CSSProperties}){
 ...
 return Component?<Component className="ui-icon" data-icon={name} style={style} size={24} stroke={1.75} aria-hidden="true"/>:<svg className="ui-icon" data-icon={name} style={style} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d={roofPaths[name]??roofPaths['flat-roof']}/></svg>;
}
